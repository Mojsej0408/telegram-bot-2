...
def start(update: Update, context: CallbackContext):
    ...
    if user_id in user_state and user_state[user_id].get("is_running"):
        ...
        context.bot.send_message(user_id, "🛑 Предыдущий пиар остановлен.")
...
def button_handler(update: Update, context: CallbackContext):
    ...
    if data.startswith("game_"):
        ...
        context.bot.send_message(user_id, "🚀 Пиар запущен")
        threading.Thread(
        ...
        context.bot.send_message(user_id, "🛑 Пиар остановлен")
    ...
    while state.get("is_running"):
        ...
        for group_id in state["groups"]:
            try:
                vk.wall.post(
                    owner_id=group_id,
                    message=add_random_emoji(state["text"])
                )
                results.append(f"✅ {abs(group_id)}")
            except Exception as e:
                results.append(f"❌ {abs(group_id)}: {e}")
